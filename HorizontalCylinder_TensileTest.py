...
while num_incl < max_incl:
    ...
    random_y = random.uniform(-L / 2 + 2*r, L / 2 - 2*r)
    random_z = random.uniform(max(length_lognormal), B - max(length_lognormal))
...
# E_vert = 0
# for i in range(len(length_lognormal)):
E_vert = 2*len(length_lognormal)*L*E1*E2/((2*L-np.pi*r)*E2 + np.pi*r*E1)
E_net = 2*(sum(length_lognormal))*r*E_vert/(B*W) + (1 - (2*(sum(length_lognormal))*r/(B*W)))*E1
