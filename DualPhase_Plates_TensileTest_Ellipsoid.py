...
while volume_fraction < volume_fraction_total:
    ...
    if fresh_lognormal_length > max_length or fresh_lognormal_length < min_length:
        continue
...
# Rotation matrix
def matrix_11(psi, theta, phi):
    return math.cos(phi) * math.cos(theta)
def matrix_12(psi, theta, phi):
    return math.cos(phi) * math.sin(theta) * math.sin(psi) - math.sin(phi) * math.cos(psi)
def matrix_13(psi, theta, phi):
    return math.cos(phi) * math.sin(theta) * math.cos(psi) + math.sin(phi) * math.sin(psi)
def matrix_21(psi, theta, phi):
    return math.sin(phi) * math.cos(theta)
def matrix_22(psi, theta, phi):
    return math.sin(phi) * math.sin(theta) * math.sin(psi) + math.cos(phi) * math.cos(psi)
def matrix_23(psi, theta, phi):
    return math.sin(phi) * math.sin(theta) * math.cos(psi) - math.cos(phi) * math.sin(psi)
def matrix_31(psi, theta, phi):
    return -math.sin(theta)
def matrix_32(psi, theta, phi):
    return math.cos(theta) * math.sin(psi)
def matrix_33(psi, theta, phi):
    return math.cos(theta) * math.cos(psi)

def rotation_matrix(psi, theta, phi):
    return [[matrix_11(psi, theta, phi), matrix_12(psi, theta, phi), matrix_13(psi, theta, phi)],
            [matrix_21(psi, theta, phi), matrix_22(psi, theta, phi), matrix_23(psi, theta, phi)],
            [matrix_31(psi, theta, phi), matrix_32(psi, theta, phi), matrix_33(psi, theta, phi)]]
...
while (num_incl < max_incl):
    ...
    translation = [random_x, random_y, random_z + b/2]
    l = length_lognormal[len(x_coordinate)]

    # point_list = []
    # point_object_list = []
    #
    # point_1 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    #                  vertex_1(w, length_lognormal[len(x_coordinate)], b))) + np.array(translation))
    # point_list.append(point_1)
    # # print(point_1)
    # point_2 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    #                  vertex_2(w, length_lognormal[len(x_coordinate)], b))) + np.array(translation))
    # point_list.append(point_2)
    # point_3 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    #                  vertex_3(w, length_lognormal[len(x_coordinate)], b))) + np.array(translation))
    # point_list.append(point_3)
    # point_4 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    #                  vertex_4(w, length_lognormal[len(x_coordinate)], b))) + np.array(translation))
    # point_list.append(point_4)
    # point_5 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    #                  vertex_5(w, length_lognormal[len(x_coordinate)], b))) + np.array(translation))
    # point_list.append(point_5)
    # point_6 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    #                  vertex_6(w, length_lognormal[len(x_coordinate)], b))) + np.array(translation))
    # point_list.append(point_6)
    # point_7 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    #                  vertex_7(w, length_lognormal[len(x_coordinate)], b))) + np.array(translation))
    # point_list.append(point_7)
    # point_8 = list(np.array(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian),
    ...
    #     psi.append(random_psi)
    #     theta.append(random_theta)
...
# params_list = [x_coordinate, y_coordinate, z_coordinate, psi, theta, phi]
...
# np.savetxt('plate_params_data.csv', params_array_T, delimiter=',')
    ...
    u1 = list(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian), [1, 0, 0]))
              # + np.array(translation))
    u2 = list(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian), [0, 1, 0]))
              # + np.array(translation))
    u3 = list(np.dot(rotation_matrix(random_psi_radian, random_theta_radian, random_phi_radian), [0, 0, 1]))
              # + np.array(translation))
    P_new = P(u1, u2, u3)
    D_new = D(w, l, b)
    A_new = A(P_new, D_new)
...
# ANALYTICAL CALCULATIONS FOR EFFECTIVE YOUNG'S MODULUS
x = max(b, w)/min(b, w)
y = mean/min(b, w)
K = [0, 0, 0]
a1 = 0.5*(3*K[0] - 2*K[2] + K[1])
b1 = K[2] - K[1]
d1 = 0.5*(K[1] - K[0])
K_final = a1 + b1*x + d1*y
K_0 = [0, 0, 0]
A0 = [-4.5, ]
B0 = [-1.688]
C0 = [-1.125]
K_infi = [0, 0, 0]
A_infi = [1.361, 1.508, 1.462]
B_infi = [2.773, 3.408, 3.192]
C_infi = [10.526, 13.668, 12.551]
a_infi = (5*A_infi*B_infi + 3*A_infi*C_infi - 8*B_infi*C_infi)/(8*A_infi - 3*B_infi - 5*C_infi)
b_infi = 10*(A_infi*B_infi - 3*A_infi*C_infi + 2*B_infi*C_infi)/(8*A_infi - 3*B_infi - 5*C_infi)
d_infi = 10*(3*B_infi - 2*A_infi - C_infi)/(8*A_infi - 3*B_infi - 5*C_infi)
